...
class SimilarTvShowsUseCase(
    ...
    fun <ViewState : ViewStateInterface> getResults(
        ...
        val result = safeApiCall(IO) {
            networkDataSource.getSimilarTvShows(
                tvShowId, 1
            )
        }
            ...
            override suspend fun handleSuccess(responseValue: TvShowSearchResponse?): DataState<ViewState>? {
                ...
                responseValue?.let {
                    return if (responseValue.results.isNullOrEmpty())
                        DataState.data(
                            response = Response(
                                message = RETRIEVING_SIMILAR_TV_SHOWS_FAILED,
                        ...
                        viewState.setData(
                            hashMapOf(
                                USE_CASE_SIMILAR_MEDIA to responseValue.results
                            )
                        )
                        DataState.data(
                            response = Response(
                                message = RETRIEVING_SIMILAR_TV_SHOWS_SUCCESSFUL,
                                messageType = MessageType.Success,
                                uiComponentType = UIComponentType.None
                            ),
                            data = viewState,
    ...
    companion object {
        const val RETRIEVING_SIMILAR_TV_SHOWS_SUCCESSFUL =
            "Successfully retrieved similar tv shows."
        const val RETRIEVING_SIMILAR_TV_SHOWS_FAILED =
            "Failed retrieving similar tv shows."
    }
}